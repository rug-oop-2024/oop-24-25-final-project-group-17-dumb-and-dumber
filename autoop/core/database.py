...
class Database:
    ...
    def _load(self):
        ...
        self._data = {}
        for key in self._storage.list_paths(""):
            collection, item_id = key.split(os.sep)[-2:]
            data = self._storage.load(f"{collection}{os.sep}{item_id}")
            # Ensure the collection exists in the dictionary
            if collection not in self._data:
                self._data[collection] = {}
            self._data[collection][item_id] = json.loads(data.decode())
